...
# G-Max Aura Sphere
#===============================================================================
# Sets up Fighting Zone + sharply raises user's defense and sp. defense.
#-------------------------------------------------------------------------------
class PokeBattle_Move_D024 < PokeBattle_MaxMove
  def pbOnStartUse(user,targets)
	user.eachOpposing do |b|
	  b.effects[PBEffects::Foresight] = true
	end
  end
	  
  def pbEffectGeneral(user)
    if @battle.field.zone!=:Fighting
      @battle.pbStartZone(user,:Fighting)
    end
