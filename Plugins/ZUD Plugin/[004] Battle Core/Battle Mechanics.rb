...
class PokeBattle_Battle
  ...
  def pbCanDynamax?(idxBattler)
    ...
    side  = battler.idxOwnSide
    owner = pbGetOwnerIndexFromBattlerIndex(idxBattler)
    return false if $game_switches[Settings::NO_PLAYER_DYNAMAX]
  ...
  def pbCanEnemyDynamax?(idxBattler)
    battler = @battlers[idxBattler]
    side  = battler.idxOwnSide
    owner = pbGetOwnerIndexFromBattlerIndex(idxBattler)
    return false if $game_switches[Settings::NO_DYNAMAX]      # No Dynamax if switch enabled.
...
class PokeBattle_AI
  ...
  def pbEnemyShouldDynamax?(idxBattler)
    battler = @battle.battlers[idxBattler]
    if @battle.pbCanEnemyDynamax?(idxBattler) && battler.pokemon.trainerAce?
