...
module Game
    ...
    if save_data.empty?
      ...
      # Reinitialize settings from the shared system files, if it exists. (Added by: kraegon)
      $PokemonSystem = load_data(SYSTEM_SETTINGS_FILE) if pbRgssExists?(SYSTEM_SETTINGS_FILE)
    end
      # Set resize factor
...
class PokemonSaveScreen
    ...
    msg = _INTL("What do you want to do?")
        ...
        cmds = []
				...
				cmds << _INTL("New Save File")
				cmds << _INTL("Old Save File")
				cmds << _INTL("Cancel")
				cmd2 = pbCustomMessageForSave(msg,cmds, ($storenamefilesave.nil? && count>0 ? 3 : 4 ))
        if !$storenamefilesave.nil? && count>0 # if true, there is pre-existing save data
          case cmd2
